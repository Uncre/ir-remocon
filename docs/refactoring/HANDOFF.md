# Detailed handoff — ir-remocon

ESP32 + FastAPI による自宅用スマート赤外線リモコン。
**現在リファクタリング作業中。フェーズごとにチャットを分けて進めている。**

この文書は、旧 `AGENTS.md` に蓄積されていた詳細な履歴・検証結果・実機手順を
Codex から参照できる形で保存した正本である。短い強制ルールはリポジトリルートの
`AGENTS.md`、読み順は `docs/refactoring/README.md` を参照すること。

Phase の計画・実装・レビューを担当する場合は、この文書を最後まで読むこと。
局所的な作業では、ルート `AGENTS.md` の指示に従って関連節を読むこと。
...
計画の全文: `docs/refactoring/plans/master-plan.md`
...
計画の全文: `docs/refactoring/plans/phase-2-plan.md`
...
計画の全文: `docs/refactoring/plans/phase-3-plan.md`
...
計画の全文: `docs/refactoring/plans/phase-4-plan.md`
...
計画の全文: `docs/refactoring/plans/phase-5-plan.md`
...
計画の全文: `docs/refactoring/plans/phase-6-plan.md`
...
- `MIN_ACCEPT_RAW_LEN = 24` と `irrecv.setUnknownThreshold()` で短い環境ノイズを除外。
- 採用できない捕捉やバッファ溢れでは学習セッションを終了せず、`irrecv.resume()` で待受を継続。
...
#### 2026-09-02 実機で判明した学習不安定の主因

- 参考回路は受信モジュールの OUT を **GPIO13** へ接続していたが、firmware v2.1.0 は
  `IR_RECV_PIN = 15` になっていた。ファームは受光部ではなく未接続の GPIO15 を読んで
  いたため、受光部を取り外しても挙動が変わらない状態だった。
- `5d2b4aa GPIOピン番号を修正` で `IR_RECV_PIN = 13` に揃え、ユーザーが実機で
  学習動作を確認した。長年の「学習が不安定」の主因はこの配線とファームの不一致だった。
- この判明により、GPIO15 の浮遊入力を観測して追加した可能性が高い v2.1.0 の
  `MIN_ACCEPT_RAW_LEN = 24`、`setUnknownThreshold()`、reject 統計は再評価対象になった。
- firmware `2.2.0` では上記の最小長フィルタと reject 統計だけを外した。
  Phase 6 本体の先頭マーク修正、配列境界、溢れ検知、採用不能時の待受継続は維持している。
  `uv run pytest -q` は 226 件、`uv run pytest -m integration -v` は 11 件が成功し、
  PlatformIO build も成功（RAM 49,772 bytes / Flash 1,057,229 bytes）。
- `2.2.0-rc1` で、無入力時に誤学習しないこと、実リモコンの学習から再送信まで
  正常に動作することをユーザーが確認した。この結果を受けて `2.2.0` として正式化した。
...
- **主因は実配線が GPIO13、ファーム設定が GPIO15 だった不一致と判明し、
  `5d2b4aa` で修正済み。** 下記はそれ以前の調査経緯として残す。
